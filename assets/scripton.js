...
const id_friend = document.querySelector('input[name="index_friend"]');

const party_g = document.querySelector('input[name="code_party_g"]');
const party_y = document.querySelector('input[name="code_party_y"]');
const rows = 8;
const cols = 8;
...
// let  whitekpiece = [];
let pieces = [];
const hiddenColor = document.querySelector('input[name="hidden_color"]');
const Color1 = hiddenColor.value;
let Color2='';
if(Color1=='white'){
    Color2='black';
}else{
...
function sendArray(Color1) {
    var  arr = JSON.stringify(pieces);
    if(party_g!=null){
        var url = 'http://localhost/home/control/checkstate.php?arr=' + encodeURIComponent(arr) + '&i=' + encodeURIComponent(id_user) + '&t=' + encodeURIComponent(id_friend) + '&g=' + encodeURIComponent(party_g);
    }else{
        var url = 'http://localhost/home/control/checkstate.php?arr=' + encodeURIComponent(arr) + '&i=' + encodeURIComponent(id_user) + '&t=' + encodeURIComponent(id_friend) + '&y=' + encodeURIComponent(3) + '&a=' + Color1;
    }
    location.href = url;
    // fetch('control/checkstate.php', {
    //     method: 'POST',
    //     headers: {
            ...
            for (let col = 0; col < cols; col++) {
                ...
                pieces.forEach(piece => {
                    if (piece.position.row == row && piece.position.col == col) {
                        ...
                        pieceElement.dataset.king = piece.isKing;
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                if (row < 3 && (row + col) % 2 !== 0) {
                    addPiece(cell, Color2);
                } else if (row > 4 && (row + col) % 2 !== 0) {
                    addPiece(cell, Color1);
                }
...
function CheckAllPiece(){
    pieces = [];
    for (let rowe = 0; rowe < rows; rowe++) {
        for (let cole = 0; cole < cols; cole++) {
            const cell = document.querySelector(`.cell[data-row="${rowe}"][data-col="${cole}"]`);
            const piece = cell.querySelector(".piece");
            if(piece){
                pieces.push({
                    color: piece.dataset.color,
                    position: { row: rowe, col: cole },
                    isKing: piece.dataset.king
                });    
            }
        }
    }
...
function onCellClick(e) {
    ...
    else if ((piece && piece.dataset.color == turn) && turn==Color1) {
        if (checkAllPiece()) {
